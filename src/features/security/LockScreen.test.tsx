 ...
 * pattern or PIN" button, the static SVG lock icon is present, and the
...
describe('LockScreen', () => {
  ...
  it('renders the static padlock icon and no "Clear" button', async () => {
    const r = await rendermount(<LockScreen {...props()} />)
    try {
      // Static closed-lock SVG from the shared icon system — no transition,
      // no animated shackle, no transform.
      expect(r.html()).toContain('viewBox="0 0 24 24"')
      expect(r.html()).toContain('aria-label="Locked"')
      expect(r.html()).not.toContain('transition: transform')
